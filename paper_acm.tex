...
accuracy using only submission metadata. The platform's execution design means that CE, TLE, and MLE verdict types are directly encoded in execution metadata (near-ceiling ROC-AUC $\geq$ 0.98), while the WA$\leftrightarrow$RE boundary is not metadata-separable (RE F1 = 0.52). These findings quantify the boundary where source-code analysis becomes necessary and where it does not, informing the design of hybrid ML--LLM feedback systems for programming education platforms. We release the dataset, pipeline, and actionable deployment guidelines for privacy-preserving educational analytics.
...
without source-code access. The residual boundary---distinguishing a Wrong
Answer from a Runtime Error---where the feedback strategy differs (algorithmic revision versus boundary-condition checking). Our study is organized
...
\begin{enumerate}
 ...
 boundary---
which determines whether feedback should target algorithmic strategy versus
boundary-condition checking---is not separable
...
variant, 92.0\%) is statistically superior to both LLMs, while GB and RF show no significant difference (p = 0.045, Bonferroni corrected): DeepSeek-V3
