...
95.02\% accuracy extends correct classification to the large majority of submissions (driven by the dominant WA class at 76.6\%, F1=0.97; Runtime Error (RE, 4.9\% of submissions) has F1=0.52 with a 49.6\% RE$\rightarrow$WA misclassification rate, indicating metadata alone is insufficient for this verdict type).\nImportantly, the platform-encoding property is a scoping result rather than a discovery: CE/TLE/MLE verdicts are defined by platform thresholds, making them inherently recoverable from execution metadata alone. Our contribution quantifies where this recovery succeeds (CE, TLE, MLE with F1 $\ge$0.89) and where it fails (WA$\leftrightarrow$RE boundary, RE F1 = 0.52). This tells practitioners when metadata is sufficient and when source-code analysis becomes necessary, rather than claiming that ML 'discovers' relationships already encoded in the platform design.
...
The ablation results (Table~\ref{tab:ablation}) reveal a clear feature hierarchy. After applying Holm-Bonferroni correction for seven pairwise comparisons (adjusted $\alpha' = 0.05/7 \approx 0.007$), all reported $p$-values ($< 0.001$) remain significant, confirming the robustness of each finding. The dominant feature is \texttt{time\_consumed\_ms}: removing it collapses accuracy by 8.8\,pp (from 95.02\% to 86.19\%), establishing execution time as the primary predictor. The second tier comprises \texttt{user\_success\_rate} (3.6\,pp drop) and \texttt{memory\_kb} (1.40\,pp drop); these two features contribute similarly-sized effects that are substantially smaller than time but larger than the remaining four features, which collectively contribute $< 1$\,pp. Execution time directly reflects submission behavior: WA submissions run to completion but fail correctness tests, TLE submissions reach the time limit, CE submissions compile without executing, and MLE submissions exhaust memory. The \texttt{user\_success\_rate} feature captures competitive programmers' historical performance, with lower success rates associated with WA or RE submissions. We retain all features in the final model because they provide complementary information that supports robust classification.
\begin{figure}[htbp]
 \centering
...
significantly outperform DeepSeek-V3 ($p < 0.001$) and \texttt{Qwen2.5:3B}
($p < 0.001$), and DeepSeek-V3 significantly outperforms \texttt{Qwen2.5:3B}
($p < 0.001$).
...
\multicolumn{4}{l}{\small \textit{Robustness note:} Holm-Bonferroni sequential correction (not shown) agrees on 9/10 comparisons; only GB vs.\ RF differs (Bonferroni: n.s.; Holm: significant at $\alpha=0.05$). We report Bonferroni as the conservative standard.}\\
\bottomrule
\end{tabular}
}%
\end{table}
...
This paper presented a controlled comparative analysis of traditional machine
learning (ML) and large language model (LLM) approaches for programming verdict
