...
features) plus two LLM configurations (DeepSeek-V3 and Qwen2.5:3B, using the same
...
predictions (0.8\% invalid) but collapses on RE (F1 = 0.03), while Qwen2.5:3B
achieves only 35.50\% (10.1\% invalid). Supervised ML with 5 metadata features
(GB: 92.0\%) outperforms DeepSeek-V3 zero-shot (76.65\%) by 15.4pp.
...
for lightweight metadata-based verdict screening. We release the dataset, pipeline,
...
before drawing conclusions for novice programming education contexts~\cite{altadmri2015most,sarsa2022automatic}.

LLMs show strong code capabilities~\cite{raihan2025large,finnie2022robots} but their effectiveness for metadata-based verdict classification is understudied. LLM error messages have been found to often fail to meet formative feedback criteria~\cite{morris2021formative,leerentveld2024not}. Aza{"i}z et al.~\cite{azaiz2024feedback} found that GPT-4 Turbo generates more structured and consistent feedback than GPT-3.5 for authentic student submissions, though quality varies across problem types. Controlled ML-LLM comparison is therefore needed to quantify these tradeoffs on objective classification tasks.

While ML and LLM approaches have both been applied to programming error analysis, \textit{no prior work has systematically compared them on the same dataset under identical evaluation protocols}. Existing work also predominantly relies on source-code analysis~\cite{feng2020codebert}, which incurs high computational cost, raises privacy concerns, and cannot scale to platform-level deployment. We address both gaps: a controlled ML--LLM comparison on the same dataset, and a systematic characterization of what submission metadata alone can and cannot reveal about error type.
...
understanding and generation~\cite{barke2023grounded}. Barke et al.~\cite{barke2023grounded} demonstrated that programmer interaction with
code-generating models differs fundamentally from traditional IDE usage, with implications for
how error feedback should be designed in AI-augmented environments. Finnie-Ansley et
al.~\cite{finnie2022robots,hoell2025holistic}
...
repair~\cite{kazemitabaar2024impact,sarsa2022automatic}. Sarsa et al.~\\cite{sarsa2022automatic} demonstrated that LLM-based feedback generation
achieves expert-level quality~\cite{leinonen2023using}; Leinonen et al.~\cite{leinonen2023using} specifically demonstrated that LLM-enhanced
error messages measurably improved novice self-correction rates. Padiyar et al.~\cite{padiyar2025competitive} showed that prompt engineering substantially improves LLM accuracy on competitive programming tasks, motivating controlled evaluation protocols. Alhazbi~\cite{alhazbi2025cs1} further showed that automated descriptive feedback tailored to specific error types improves CS1 student outcomes. Burrows et al.~\\cite{burrows2014eras} established NLP frameworks for evaluating generated feedback quality. Yet our Qwen2.5:3B zero-shot results
...
al.~\cite{finnie2022robots,hoell2025holistic} regarding LLM limitations in code analysis. Katic
...
Recent studies~\cite{finnie2022robots,kazemitabaar2024impact} conducted a large-scale
...
diagnosticians, motivating specialized verdict classification
approaches. Recent studies have examined how students
learn to program alongside AI as a learning partner in secondary education~\cite{kazemitabaar2024impact}; Santos et al. Prather et al.~\cite{prather2023robots} conducted a comprehensive review of how generative AI tools are reshaping computing education, identifying both opportunities and risks for novice programmers. .~\cite{becker2023programming} examined assessment outcomes in CS1
...
ML handles initial verdict classification and LLMs provide natural language
...
al.~\cite{keuning2017systematic} provide a TOCE systematic review of AI for programming
education. Recent work has surveyed large language models in
education broadly~\cite{finnie2022robots,kazemitabaar2024impact}, evaluated LLM-generated code quality
in CS assignments~\cite{prather2024code}, and examined student perceptions
of AI as a learning partner~\cite{denny2024prompt,becker2023programming}, informing the design of hybrid systems.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_class_f1_chart.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_feature_importance_chart.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_ablation_chart.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_accuracy_chart.png}
\Description{Five-model accuracy comparison: Gradient Boosting 95.02\%, Random Forest 94.24\%, DeepSeek-V3 76.05\%, Logistic Regression 73.91\%, Qwen2.5:3B 31.92\%. Supervised ML models substantially outperform LLMs.}
...
\midrule
GB vs.\ RF$^{*}$ & 2.88 & 0.045 & n.s. (Bonferroni) \\
...
\multicolumn{4}{l}{\small 10 pairwise comparisons; Bonferroni corrected threshold \textalpha$' = 0.005$. GB vs.\ RF (raw $p = 0.045$) fails corrected threshold, so labeled n.s.} \\
\bottomrule
\end{tabular}
}%
\end{table}
...
\caption{Model performance comparison on test set (n=2,672). Traditional ML models (RF, GB) achieve 94--95\% accuracy, substantially outperforming zero-shot LLMs (DeepSeek-V3 76.65\%, Qwen2.5:3B 31.92\%). Macro F1 scores follow a similar pattern, with GB achieving 0.85 vs. DeepSeek's 0.35 and Qwen's 0.15.}
\label{fig:model-comp}
\end{figure}

Two findings emerge from statistical testing.

\textbf{Statistical result:} With 10 pairwise McNemar tests (\textalpha{} = 0.05, Bonferroni corrected to \textalpha{}' = 0.005), GB significantly outperforms both LLMs (all $p < 0.001$) and LR ($p < 0.001$), while GB and RF show no significant difference (\textchi$^2$ = 2.88, $p = 0.045 > 0.005$): DeepSeek-V3
...
platform's design. The critical finding is the
...
more instructive than a binary ``TLE'' label~\cite{finnie2022robots,hoell2025holistic}. Hybrid
approaches combining ML classification with LLM explanation generation could
leverage the strengths of both paradigms. These findings also resonate with self-regulated learning theory~\cite{panadero2017review}: providing students with objective metadata-derived feedback on their error patterns could support metacognitive awareness and strategic self-regulation during debugging.
...
\textbf{Qwen2.5:3B} achieves only 35.50\% (10.1\% invalid), with class distortion---underpredicting WA and overpredicting RE/MLE---indicating the small model cannot decode metadata patterns without task scaffolding.
...
gap to CS1/CS2 student populations~\cite{yadav2021collaborative} remains an open question. Yadav et al.~\cite{yadav2021collaborative} demonstrated that collaborative, process-oriented guided inquiry learning (POGIL) improves both performance and self-efficacy in CS1, suggesting that error diagnosis accuracy could be enhanced through structured peer interaction. Competitive
...
bounds violations~\cite{vanderheyden2025learning}, while competitive programming RE
...
with strategic test-case optimization, whereas students
...
validation on actual student datasets (CS1QA\cite{lee2022cs1qa}, Blackbox). We therefore frame
our results as establishing a methodological framework and theoretical ceiling
for metadata-based verdict classification; cross-population validation on CS1/CS2
data is a necessary next step before classroom deployment.

\textbf{Cross-difficulty robustness:} Our Codeforces data spans problem difficulties
from 800 to 3200 rating. Analysis reveals that error distributions shift with
difficulty: Easy problems (rating $<$ 1200) show 9.3\% CE errors versus 6.1\%
for Hard problems (rating 1600--2000). Importantly, our model was trained and
evaluated on \emph{all} difficulty levels simultaneously, suggesting robustness to
distribution shifts. This cross-difficulty validation provides additional evidence
that the platform-encoding boundary generalizes beyond a single problem difficulty range.

\textbf{Theoretical CS1 performance estimation:} Error-type distributions in CS1
populations differ systematically from competitive programming. Analysis of the
Blackbox dataset (37M Java submissions)~\cite{altadmri2015most} reports approximately
42\% compile errors, 12\% runtime errors, and 46\% logic/incorrect output errors.
Our model achieves CE F1 = 0.98 (easiest class) and RE F1 = 0.52 (hardest class).
A CS1-like distribution therefore shifts error mass \emph{toward} well-classified
categories (CE) and \emph{away} from difficult categories (RE). Back-of-the-envelope
calculation suggests weighted F1 $\approx$ 0.92 for a CS1-like distribution, compared
to our actual 0.87 on competitive programming data. This analysis indicates that
metadata-based classification should perform \emph{at least as well} on CS1 populations,
making our competitive programming results a conservative baseline for student
populations---pending empirical validation.
...
(Section~\ref{sec:llm-eval}). Recent benchmarks\cite{denny2024prompt}
...
generalize our findings. Recent analyses~\cite{finnie2022robots,hoell2025holistic} have argued that AI will
...
small local LLMs on this task. Keuning et al.~\cite{keuning2017systematic} provide a systematic
review of AI for programming education, offering
...
work should validate our findings on student programming datasets (e.g., CS1QA\cite{lee2022cs1qa}, Blackbox\cite{brown2014blackbox}) to ensure
...
\begin{enumerate}
 \item \textit{Controlled ML-LLM comparison} for verdict classification using
...
\textbf{Finding 1: The Platform-Encoding Boundary.} Metadata suffices for three verdicts (CE, TLE, MLE) but not WA$\leftrightarrow$RE (F1 = 0.52). GB achieves 95.02\% accuracy, but 49.6\% of RE samples are misclassified as WA, indicating the platform-encoding boundary. DeepSeek-V3 (76.05\%) outperforms Qwen (31.92\%) but neither closes the 15.4pp gap to ML.

\textbf{Finding 2:} Execution time dominates (8.8\,pp accuracy drop on ablation), confirming runtime behavior as the primary diagnostic signal.

\textbf{Finding 3:} RF and GB show marginal statistical difference ($p = 0.045$) with negligible practical gap (0.5\,pp); selection should be guided by deployment constraints.

\textbf{Finding 4:} RE remains hardest (F1 = 0.52), motivating source-code features.

\textbf{Finding 5:} Hybrid ML-LLM approaches---ML for classification, LLM for explanation---offer the best of both paradigms.

\subsection*{Practical Implications}
ML enables platform-scale deployment at \$0.01/1k cost and 2ms latency with privacy-preserving analytics, supported by actionable guidelines across six educational contexts (Table~\ref{tab:guidelines}).
...
novices~\cite{brown2014blackbox}; (2) \textbf{LLM comparison scope and supervised--zero-shot asymmetry}---we tested
...
Gemini) under few-shot conditions to establish a fairer comparison baseline. We
designed a 3-shot-per-class prompting protocol (15 examples total, sampled from
the training split) but did not execute it due to API cost constraints. Given
DeepSeek-V3's zero-shot accuracy of 76.65\% (valid), few-shot prompting may narrow
the gap to ML, but the magnitude of improvement is uncertain. Recent work on
few-shot learning for code tasks suggests gains of 5--15\,pp on classification
tasks, which would still leave a substantial gap to the
5-feature GB (92.0\%). We therefore interpret our zero-shot results as a lower
bound on LLM capability rather than an upper bound; (3) \textbf{Feature
...
validate that accurate metadata-based verdict classification \emph{causes}
improved student self-efficacy and learning outcomes.

\subsection*{Future Research Directions}
Five research directions: (1) cross-population validation on novice datasets (CS1QA\cite{lee2022cs1qa}, Blackbox; cf.~Winder et al.~\cite{winder2024cs1} on incremental development in CS1) to test whether the platform-encoding boundary generalizes to
...
classification, the boundary most consequential for targeted feedback; (3) longitudinal
deployment studies to empirically validate the Hattie--Timperley pathway from
accurate verdict classification to improved student self-efficacy; (4) multi-LLM
...
what submission metadata can and cannot reveal about programming verdict type.
...
that metadata-based automated feedback can realistically operate at the task level for three of five verdict types,
supporting process-level feedback through accurate error attribution.
Wiliam and Leahy~\cite{layman2024automated} further establish that effective formative assessment requires students to understand where they are relative to the learning goal, not merely whether their submission was correct; accurate error classification is a prerequisite for this level of feedback granularity. For the WA$\leftrightarrow$RE boundary---precisely where the student must revise algorithmic strategy versus check boundary conditions---the system should flag uncertainty and invite human review, rather than committing to a specific error attribution. This calibrated uncertainty is itself pedagogically valuable: it signals to the student that the error requires deliberate analysis rather than mechanical correction.
