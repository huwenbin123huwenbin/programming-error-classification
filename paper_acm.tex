...
The compilation-time and resource-exhaustion properties of the platform's execution environment mean that these three verdict types are near-deterministically recoverable from execution metadata alone (F1 $\ge$0.89 for CE, TLE, MLE). The scientifically and educationally interesting
...
Introductory programming courses report 30--40\% dropout rates~\cite{bennedsen2007failure,watson2014failure}. Debugging difficulty is a primary obstacle~\cite{watson2014failure}, as novices struggle to distinguish error types---misinterpreting runtime errors as logical errors~\cite{spohrer1986novice}.

Prior work analyzed compilation behavior~\cite{jadud2006methods} and scaled to thousands of users via Blackbox~\cite{brown2014blackbox}, revealing systematic debugging differences between student groups.
...
(2) evaluate only LLM methods~\cite{raihan2025large,finnie2022robots},
...
embeddings)~\cite{feng2020codebert}, which incurs high
...
\begin{enumerate}
 ...
 consistent evaluation protocol, establishing the relative accuracy--cost
 tradeoffs for each approach.
 \item \textbf{Decomposition of metadata signal}: we systematically quantify
 how much error-type information is genuinely recoverable from execution
 metadata alone, demonstrating that CE, TLE, and MLE are near-deterministically
 separable (F1 $\ge$ 0.89) because the platform's resource-limit design
 operationally encodes them, while the WA$\leftrightarrow$RE boundary
 is not separable from metadata alone (RE F1 = 0.52) because neither
 error type produces distinctive execution signatures. This boundary
 precisely demarcates where source-code analysis becomes necessary.
 \item \textbf{Quantitative guidelines} for platform designers: execution time
 and memory are the dominant metadata predictors (8.8 pp accuracy drop
 when removed), and the statistical analysis (McNemar tests, ablation,
 Gini importance, confusion matrices) quantifies the accuracy--cost--latency
 tradeoffs across six deployment contexts. We release the dataset, pipeline,
 and experiment code at \url{https://github.com/huwenbin123huwenbin/programming-error-classification}.
\end{enumerate}
