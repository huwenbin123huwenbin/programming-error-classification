...
We evaluate Random Forest, Gradient Boosting, and Logistic Regression (7 metadata
features) plus two LLM configurations (DeepSeek-V3 and \texttt{Qwen2.5:3B}, using the same
5 metadata features as text), under identical zero-shot protocols. Significance is
assessed with McNemar's test with Bonferroni correction.
...
training. The 15.4pp gap reflects the supervised ML advantage; few-shot experiments
(Section 4.5) demonstrate that LLM performance is sensitive to prompting strategy,
with 5-shot reducing accuracy to 64.55\% but improving Macro F1 from 0.3467 to 0.6266.
Supervised ML with 5 metadata features (GB: 92.0\%) outperforms DeepSeek-V3 zero-shot
(76.65\%) by 15.4pp.

\noindent\textbf{Conclusions}: We characterize the platform-encoding boundary
for verdict classification from submission metadata. CE, TLE, and MLE are
directly determined by platform thresholds (ROC-AUC $\geq$ 0.98), while
WA$\leftrightarrow$RE is not metadata-separable (RE F1 = 0.52). These findings
quantify where source-code analysis becomes necessary, providing empirical grounding
...
Introductory programming courses report 30--40\% dropout rates~\cite{bennedsen2007failure,watson2014failure}, with debugging difficulty as a primary obstacle~\cite{watson2014failure,spohrer1986novice}. Prior work analyzed compilation behavior~\cite{albrecht2020sometimes} and scaled to large populations via Blackbox~\cite{brown2014blackbox}, revealing systematic debugging differences between student groups. Competitive programming platforms such as Codeforces and LeetCode provide structured metadata (execution time, memory consumption, precise error verdicts, test cases passed) for every submission, making them valuable environments for studying programming error patterns at scale.
...
Spohrer and Soloway~\cite{fwa2024experience} provided an influential early
...
Robust behavioral modeling distinguishes productive from unproductive debugging~\cite{effenberger2021validity}, aligning with cognitive load principles~\cite{sweller2011cognitive}. Process mining approaches~\cite{tang2025edm} highlight the value of analyzing interaction sequences over aggregated statistics.
...
Jadud~\cite{albrecht2020sometimes} showed that compilation frequency and
...
identification}~\cite{romangonzalez2024computational}. We are not aware of
...
inefficiencies), which Hattie ranks as the most impactful for learning. Wagner~\cite{wagner2025hattie} synthesizes Hattie and Timperley's model across educational contexts; Layman~\cite{layman2024automated} demonstrates that automated formative feedback at the task level consistently improves student learning outcomes.
...
Second, Bandura's self-efficacy theory~\cite{bandura1997self,kokinda2024informal} posits that
accurate performance feedback is a primary driver of self-efficacy beliefs~\cite{ramalingam1998validation}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{fig_ablation_chart.png}
\Description{Ablation study: accuracy drop when removing each feature. Execution Time causes the largest drop (8.80 pp), followed by User Success Rate (3.60 pp) and Memory (1.40 pp); other features cause negligible drops.}
\caption{Ablation study visualized: accuracy drop (pp) when each feature is removed individually (Gradient Boosting, test set $n=2{}672$). Execution time is dominant---removing it collapses accuracy by 8.8\,pp (95.02\% $\to$ 86.19\%)---while \texttt{user\_success\_rate} (3.6\,pp) and \texttt{memory\_kb} (1.4\,pp) are secondary; the remaining features are negligible.}
\label{fig:ablation}
\end{figure}
...
(5-shot, 15 examples) reduces DeepSeek-V3 accuracy to 64.55\% but raises Macro F1
from 0.3467 to 0.6266, demonstrating that prompting strategy fundamentally
...
with Macro F1 = 0.35 (zero-shot; detailed per-class in Table~\ref{tab:llm-analysis});
...
and Finnie-Ansley et al.~\cite{finnie2022robots,hoell2025holistic} report related LLM
...
\caption{Model performance comparison on test set (n=2,672). Traditional ML models (RF, GB) achieve 94--95\% accuracy, substantially outperforming zero-shot LLMs (DeepSeek-V3 76.65\%, \texttt{Qwen2.5:3B} 31.92\%). Macro F1 scores follow a similar pattern, with GB achieving 0.85 vs. DeepSeek's 0.35 and Qwen's 0.15.}
...
Memory consumption follows (4.2 pp), user success rate contributes 3.6 pp;
...
Despite lower accuracy, LLMs offer explainability advantages~\cite{finnie2022robots,hoell2025holistic};
a hybrid strategy (ML for classification, LLM for explanation) leverages both.
...
remains open. Competitive programmers differ from novices in error profile
(more CE, fewer complex RE)~\cite{altadmri2015most}, error cause (novice RE
dominated by null pointer/array bounds violations)~\cite{vanderheyden2025learning},
and submission behavior (iterative learning vs. competitive time pressure).
...
We tested two LLM configurations (DeepSeek-V3 and \texttt{Qwen2.5:3B}) under
zero-shot protocol (Section~\ref{sec:llm-eval}); recent benchmarks confirm
substantial LLM family variation on programming tasks. GPT-4o would cost
...
student receiving a CE verdict benefits from syntax guidance, not algorithmic
review. For the WA$\leftrightarrow$RE boundary---where the student must decide
between revising algorithmic strategy and checking boundary conditions---the system
should flag uncertainty and invite deliberate analysis; this calibrated uncertainty
is itself pedagogically valuable~\cite{layman2024automated}. Hybrid deployment---
ML for fast, cheap verdict classification (\$0.01/1k, 2ms latency), with LLMs
conditionally invoked for explanation generation at the boundary---represents the
most cost-effective path toward intelligent programming education support.
