...
of 30\% to 40\% worldwide~\cite{bennedsen2007failure,watson2014failure}. This
problem---termed the ``CS1 problem''---has been
documented over the past two decades and remains a pressing
issue in computing education~\cite{pears2007survey,liao2016lightweight}. Liao et al.~\cite{liao2016lightweight} demonstrated that lightweight, metadata-based
signals can identify at-risk students as early as the first week, enabling timely intervention. Among contributing
...
versa~. This diagnostic difficulty is compounded by
...
syntax highlighting and compilation diagnostics. Prather et al.~\cite{prather2017novices} found that novices spend significant cognitive effort
interpreting error messages; Becker et al.~\cite{becker2016compiler} demonstrated that
well-designed compiler messages measurably reduce error resolution time. Spohrer and Soloway~\cite{spohrer1986novice} observed that many student
...
debugging~\cite{raihan2025large,prather2024code,kazemitabaar2024impact,finnie2022robots}. In educational contexts, LLMs have been deployed as
...
While LLMs have demonstrated strong code synthesis capabilities, Morris et al.~\cite{morris2021formative} established through systematic review that formative
feedback is most effective when timely, specific, and actionable---criteria that current
LLM error messages often fail to meet. Leerentveld et al.~\cite{leerentveld2024not} found that LLM-enhanced error messages were ineffective in practice,
...
and are resistant to standard instructional interventions. Luxton-Reilly et al.~\cite{luxton2018introductory} conducted a systematic review of 500+ CS1 studies,
establishing error diagnosis as the most persistent novice difficulty; complementary
analysis on feedback\cite{barbosa2023feedback}
...
literature. Prior work has demonstrated that
metadata-based prediction of programming success achieves strong performance in CS1~\cite{jadud2006methods,altadmri2015most},
and learning analytics in programming education has been analyzed extensively~\cite{romero2013data,baker2009educational}. Piech et al.~\cite{piech2015deep} established
...
Prior studies have shown that features derived from early programming
...
accuracy. These results indicate that behavioral
features from the first four weeks of a semester are sufficient to identify
students at risk of failure, enabling early intervention~\cite{jadud2006methods,brown2014blackbox}. Jayaprakash et al.~\cite{jayaprakash2014early} showed that metadata-derived signals can be operationalized in instructor-facing dashboards~\cite{verbert2013learning} for timely academic alerts~\cite{pears2007survey,jayaprakash2014early,shute2013formative} for formative student monitoring~\cite{shute2013formative}.
while Ifenthaler and Yau~\cite{ifenthaler2020utilising} demonstrated that interpretable model outputs increase instructor trust and intervention uptake Jayaprakash et al.~\cite{jayaprakash2014early} demonstrated that predictive analytics dashboards can operationalize these signals for instructor-facing early alert systems.
...
productive from unproductive debugging behaviors in programming education. These findings align with cognitive load principles~\cite{sweller2011cognitive}:
high compilation frequency with low diversity may indicate germane load cycles
without effective strategy switching~\cite{kirschner2018cognitive}, a behavioral pattern. Scherer et al.~\cite{scherer2018meta} further demonstrated through meta-analytic structural equation modeling that technology acceptance in educational settings depends critically on perceived usefulness and ease of use, factors that explain the adoption gap between research prototypes and deployed systems in programming education. our ablation experiments partially capture through the time feature. Romero Romero and Ventura~\cite{romero2013data}
...
our class-imbalance results. Luxton-Reilly et al.~\cite{luxton2018introductory} corroborated these findings
across 500+ CS1 studies; Gorson and O'Rourke~\cite{gorson2020why} showed that students
systematically misattribute RE errors to algorithmic faults rather than runtime conditions and the value of models capturing non-linear
