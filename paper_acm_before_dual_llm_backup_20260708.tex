...
plus Qwen2.5:3B zero-shot prompting, under an identical 80/20 split with
...
0.52), motivating source-code integration. Qwen2.5:3B zero-shot achieves only
35.50\% accuracy on valid predictions (10.1\% invalid outputs; 31.92\%
counting all samples), substantially weaker than GB---a 59.5pp gap on
identical features.
...
\begin{itemize}
    ...
    \item \textbf{RQ3:} How does a local LLM (Qwen2.5:3B zero-shot) compare to traditional ML methods on the same task
          and dataset?
...
\begin{enumerate}
    ...
    \item \textbf{ML--LLM comparison}: No work systematically compares traditional
          ML (RF, GB, LR) and modern LLM approaches (Qwen2.5:3B)
...
\begin{itemize}
    \item \textbf{Qwen2.5:3B Zero-Shot} \textit{(evaluated)}: The
          Qwen2.5:3B model was designed to receive a structured classification
    ...
    \item \textbf{Qwen2.5:3B Few-Shot (5-shot, not evaluated)}: The same prompt was used,
...
Inference was performed using local Ollama v0.30.7 with Qwen2.5:3B
at no API cost. Grammar-constrained JSON output was used to ensure
parsable responses.
and evaluated on the same 2,672-sample test set as Qwen2.5:3B.)
...
(Qwen2.5:3B), the test set of 2,672 samples was classified directly using
...
Qwen2.5:3B experiments used local Ollama (temperature=0,
...
on the same test set of 2,672 samples, reporting our own Qwen2.5:3B
...
\centering
\small
\caption{Model performance: ML baselines vs. Qwen2.5:3B zero-shot (test set $n=2{,}672$)}
...
Qwen2.5:3B Zero-Shot (all)   & 31.92\% & [30.0, 33.9] & ---    & \$0.00 \\
Qwen2.5:3B Zero-Shot (valid) & 35.50\% & [33.6, 37.4] & 0.2311 & \$0.00 \\
\multicolumn{5}{l}{\small $^{*}$ML models: trained on 10,688 samples, evaluated on 2,672 test.}\\
\multicolumn{5}{l}{\small $^{\dagger}$Qwen2.5:3B: 269/2,672 predictions \textsc{unknown} (model did not return a valid verdict).}\\
\multicolumn{5}{l}{\small $^{\ddagger}$Few-shot not evaluated; zero-shot shown in row 1.}\\
\bottomrule
\end{tabular}%
}
\end{table}
...
substantially outperform Qwen2.5:3B zero-shot.

Our Gradient Boosting achieves 95.02\% accuracy. Qwen2.5:3B zero-shot on the
full test set (n=2,672) achieves only 31.92\% accuracy, with 269/2,672
...
\textbf{Key Finding 2:} Qwen2.5:3B zero-shot fails to recover the
metadata-derivable signal.

Qwen2.5:3B zero-shot (31.92\% all; 35.50\% valid) falls far below the ML baselines
(RF 94.24\%, GB 95.02\%). Crucially, 10.1\% of Qwen predictions
...
(31.92\%) further demonstrates that small, unrepresentative pilot samples
yield misleadingly optimistic estimates of LLM capability. We therefore
frame the Qwen result as a documented \emph{failure mode} and a
...
\textbf{Key Finding 3:} Qwen2.5:3B zero-shot faces challenges in competitive
programming error classification (full evaluation).

On the full 2,672-sample test set, Qwen2.5:3B accuracy was 31.92\% with
269/2,672 (10.1\%) invalid predictions. Prior pilot results on 207 samples
representative of the full test set.
representative of the full test set.
...
GB vs.\ Qwen2.5:3B$^{\dagger}$ & 1448.76 & $<0.001$ & Yes \\
RF vs.\ Qwen2.5:3B$^{\dagger}$ & 2144.15 & $<0.001$ & Yes \\
LR vs.\ Qwen2.5:3B$^{\dagger}$ & 1084.21 & $<0.001$ & Yes \\
\multicolumn{4}{l}{\small $^{*}$ML model comparisons: 2,672 held-out test
       samples (13,360 dataset).} \\
\multicolumn{4}{l}{\small $^{\dagger}$      GB vs.\ Qwen: $\chi^2=1448.76$, $p<0.001$; RF vs.\ Qwen: $\chi^2=2144.15$, $p<0.001$; LR vs.\ Qwen: $\chi^2=1084.21$, $p<0.001$. Qwen: 10.1\% invalid.} \\
\bottomrule
\end{tabular}
}%
\end{table}
...
statistically superior to Qwen2.5:3B ($\chi^2 = 1448.76$, $p < 0.001$)
\emph{as executed}. Because 10.1\% of Qwen outputs were invalid and
...
Gradient Boosting (95.02\%) substantially outperforms Qwen2.5:3B
(31.92\% on full test set, with 10.1\% invalid predictions). RF (\$0.01 per
1k classifications) is
approximately free local inference, making ML
...
\subsection{Qwen2.5:3B Error Analysis}

Our Qwen2.5:3B zero-shot experiments reveal limited error discrimination in
31.92\% accuracy). This 64.27 percentage point gap underscores the risk of
evaluating LLMs on small pilot samples. We identify three key factors
contributing to Qwen2.5:3B's poor performance on the full test set:

\textbf{1. Invalid Prediction Rate (10.1\%):} Out of 2,672 test samples, 835
(10.1\%) received \textsc{unknown} predictions---the model did not return a
...
\textbf{3. Verdict Distribution Mismatch:} Qwen2.5:3B's predictions show WA bias (6.2\% of valid)
...
\begin{table}[htbp]
\centering
\caption{Qwen2.5:3B prediction analysis on full test set (n=2,672)}
...
Invalid predictions (\textsc{unknown}) & 835   & 10.1\% & Model did not return valid verdict \\
\midrule
...
We tested only Qwen2.5:3B zero-shot (Section~\ref{sec:llm-eval})
as a representative small local LLM. Future work should include a broader
range of LLMs (GPT-4, Claude, LLaMA-3, CodeLlama) to
generalize our findings. Our results establish a clear performance ceiling for
small local LLMs on this task.
...
CI [94.2\%, 95.9\%]). Qwen2.5:3B zero-shot on the full test set (n=2,672)
achieves only 31.92\% accuracy, with 10.1\% invalid predictions. We interpret
...
scope---only Qwen2.5:3B was tested as a representative small local LLM; (3) feature
